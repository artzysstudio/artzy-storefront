...
let products = records
  ...
  .map((record) => {
    ...
    return {
      ...
      careInstructions: text(record.care_instructions) || undefined,
      contents: text(record.contents || record.box_contents) || undefined,
      handmadeVariation: text(record.handmade_variation) || undefined,
      dispatchTime: text(record.dispatch_time) || undefined,
      deliveryEstimate: text(record.delivery_estimate) || undefined,
      returnEligibility: text(record.return_eligibility) || undefined,
      giftWrappingAvailable: typeof record.gift_wrapping_available === 'boolean' ? record.gift_wrapping_available : undefined,
      leadTime: text(record.lead_time || record.production_lead_time) || undefined,
      seo: {
