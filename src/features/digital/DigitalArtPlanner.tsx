...
const styles = DIGITAL_ART_DIRECTION_IDS.map((id) => ART_DIRECTIONS[id]);
...
export default function DigitalArtPlanner() {
  ...
  const message = useMemo(() => [
    ...
    `Style direction: ${style?.name ?? 'Please recommend'}`,
    `Visual language: ${style?.visualLanguage ?? 'Please recommend'}`,
  ...
  return <section className="digital-planner" id="digital-planner" aria-labelledby="digital-planner-title">
    ...
    <div className="digital-planner__shell">
      <form className="digital-planner__form" onSubmit={(event) => event.preventDefault()}>
        ...
        <fieldset><legend><b>2</b><span>Which visual direction feels right?<small>Each preview now follows a distinct composition language. Folk-art choices are inspiration directions, not claims of traditional provenance.</small></span></legend><div className="digital-style-options">{styles.map((item) => <button type="button" aria-pressed={styleId === item.id} onClick={() => setStyleId(item.id)} key={item.id}><ArtDirectionMark direction={item.id}/><span><strong>{item.name}</strong><small>{item.shortNote}</small></span></button>)}</div><p className="digital-style-context" aria-live="polite">{style ? <><b>What this means:</b> {style.visualLanguage}. {style.studioNote}</> : 'Select a direction to see what visual elements the studio will interpret.'}</p></fieldset>
      ...
      <aside className="digital-planner__summary" aria-live="polite">
        ...
        <dl><div><dt>Style</dt><dd>{style?.name ?? 'Choose or ask us to recommend'}</dd></div><div><dt>Finish</dt><dd>{output?.[1] ?? 'Choose or ask us to recommend'}</dd></div><div><dt>Scale</dt><dd>{size}</dd></div><div><dt>Timing</dt><dd>{timeline}</dd></div></dl>
