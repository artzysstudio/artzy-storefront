...
const quickStarts: Array<{ label: string; note: string; intent: Partial<GiftIntent> }> = [
  { label: 'Build a gift hamper', note: 'Curated from available ERP pieces', intent: { giftType: 'hamper', budget: 3000, quantity: 1, styles: ['artistic', 'premium'] } },
...
export default function GiftBuilder({ products }: { products: Product[] }) {
  ...
  const applyQuickStart = (preset: (typeof quickStarts)[number]) => {
    ...
    setStep(preset.intent.giftType === 'hamper' ? 0 : 2);
  ...
  return <section className="gift-builder" id="gift-concierge" aria-labelledby="gift-builder-title">
    ...
    <div className="gift-builder__shell">
      ...
      <div className="gift-step-panel">
        ...
        {step === 0 && <><div className="gift-format-choice" role="group" aria-label="Gift format"><button type="button" className={intent.giftType === 'single' ? 'selected' : ''} onClick={() => set('giftType', 'single')}><b>Single gift</b><small>Choose one available studio piece</small></button><button type="button" className={intent.giftType === 'hamper' ? 'selected' : ''} onClick={() => set('giftType', 'hamper')}><b>Gift hamper</b><small>Combine available ERP pieces into a curated hamper</small></button></div><ChoiceGrid values={occasions} selected={[intent.occasion]} onSelect={(value) => set('occasion', slug(value))}/></>}
