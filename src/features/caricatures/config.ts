...
export const CARICATURE_STYLES: Record<CaricatureStyleId, CaricatureStyle> = {
  cute_cartoon:{id:'cute_cartoon',name:'Cute Cartoon',summary:'Friendly expressions, soft colour and gently enlarged heads.',bestFor:'Children, birthdays and families',image:'/images/caricature-style-cute-cartoon.webp',positive:'friendly original cute cartoon, soft colours, gently enlarged heads, warm expressive eyes',negative:'babyish distortion, plastic 3D character',strength:.62,guidance:8,compositions:ALL},
...
export function buildCaricaturePrompt(brief:CaricatureBrief){const style=CARICATURE_STYLES[brief.styleId];return {prompt:[`Create an original, respectful personalised caricature from the supplied reference photo. Style: ${style.positive}.`,`Preserve recognisable facial structure, skin tone, age range, hairstyle, clothing colours and identity. Show exactly ${brief.people} people and ${brief.pets} pets.`,`Composition: ${brief.composition.replace('_',' ')}. Occasion: ${brief.occasion.replaceAll('_',' ')}.`,`Profession: ${brief.profession||'not specified'}. Hobbies: ${brief.hobbies||'not specified'}. Preferred colours: ${brief.colours||'from reference'}. Clothing: ${brief.clothing||'from reference'}. Background: ${brief.background||'simple warm neutral'}. Props: ${brief.props||'none'}. Notes: ${brief.notes||'none'}.`,'Positive, dignified representation. Do not exaggerate disability, race, religion, body type, age or any sensitive characteristic. No words unless separately approved.'].join(' '),negative_prompt:[style.negative,'offensive stereotype, changed ethnicity, changed age, deformed face, malformed hands, duplicate subject, extra person, extra pet, missing subject, cropped head, text, letters, logo, watermark, copyrighted character'].join(', '),strength:style.strength,guidance:style.guidance};}
