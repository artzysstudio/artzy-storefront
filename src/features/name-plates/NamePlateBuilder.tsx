...
export default function NamePlateBuilder() {
  ...
  const payload = useMemo(() => ({
    ...
    estimate: { currency: 'INR', base: size.price, shape: shape.price, painting: motif.price, material: material.price, protection: protection.price, mounting: mounting.price, total: estimate, authority: 'Studio confirmation required' },
  ...
  const brief = useMemo(() => [
    ...
    `Website estimate: ₹${inr.format(estimate)}. Studio reference: ${reference || 'not created yet'}.`, 'Please confirm feasibility, final price, taxes, production time and delivery.',
  ...
  async function submitDraft() {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({ error: 'The studio service returned an incomplete response.' })) as DraftResponse;
      ...
      if (!nextReference) throw new Error('The studio service did not return a tracking reference.');
