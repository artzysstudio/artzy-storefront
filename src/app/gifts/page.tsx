...
export default async function GiftsPage() {
  ...
  return <><Header/><main className="gift-concierge-page">
    ...
    <section className="gift-seo-links"><div><span>Explore by moment</span><h2>Every reason to make someone feel seen.</h2></div><div>{['Wedding gifts','Birthday gifts','Housewarming gifts','Festival gifts','Employee gifts','Client gifts','Return gifts','Anniversary gifts'].map((label) => <a href="#gift-finder" key={label}>{label}<span>→</span></a>)}</div></section>
