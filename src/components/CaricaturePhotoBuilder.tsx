...
const finishes = [
  ...
  { id: 'framed_artwork', name: 'Framed print', base: 3490, size: 'A4 starting size', frame: 'Frame selected and quoted by studio', revisions: 'One minor studio-confirmed revision', time: '8–12 working days' },
  ...
  { id: 'gift_ready', name: 'Gift presentation', base: 4490, size: 'A4 framed starting presentation', frame: 'Presentation and frame quoted by studio', revisions: 'One minor studio-confirmed revision', time: '10–15 working days' },
];
const visualStyles = [
...
const subjectChoices: Array<{ id: SubjectId; name: string; note: string }> = [
  { id: 'one_person', name: 'One person', note: 'Individual portrait' },
  { id: 'couple', name: 'Two people', note: 'Couple or friends' },
  { id: 'family', name: 'Family', note: 'Three or four people' },
  { id: 'pet_only', name: 'One pet', note: 'Pet portrait' },
  { id: 'person_with_pet', name: 'Person + pet', note: 'Together in one scene' },
  { id: 'group', name: 'Larger group', note: 'Studio review' },
];
const caricaturePriceGuidance = { minimum: 1499, maximum: 9999 } as const;
...
export default function CaricaturePhotoBuilder() {
  ...
  const payload = useMemo(() => ({
    ...
    estimate: { currency: 'INR', minimum: caricaturePriceGuidance.minimum, maximum: caricaturePriceGuidance.maximum, label: 'indicative_range', authority: 'Artzy Studio confirmation required' }, requiredDate: requiredDate || null,
  }), [user, brief, caricatureType, style.name, finish.name, commercialUsage, permissions, shortMessage, selectedAssetId, referenceAssetId, samples, customerPrompt, exaggeration, requiredDate]);
  ...
  async function acceptPhoto(file: File, source: PhotoState['source']) {
    ...
    try { if (photo?.preview) URL.revokeObjectURL(photo.preview); await clearGeneratedAssets(); setPhoto(await preparePhoto(file, source)); setReferenceAssetId(''); setConsentId(''); setStatus('Photo ready. It stays on this device until you review consent and choose to create an ArtzyAI sample.'); setPermissions(current => ({ ...current, process: false, ai: false, handoff: false })); }
  ...
  return <section className="caricature-builder caricature-wizard" id="caricature-builder" aria-labelledby="caricature-builder-title">
    ...
    <div className="caricature-builder__panel"><div className="caricature-builder__pane-stage">
      ...
      {step === 1 && <div className="wizard-pane caricature-subject-step"><header><b>Step 2 · Subjects</b><h3>Who should appear?</h3><p>Choose the closest option, then adjust the numbers if needed.</p></header><div className="subject-price-note"><strong>Caricatures start from ₹1,499</strong><span>Final price depends on the subjects, size, print type, frame and artwork detail.</span></div><fieldset className="subject-choice-grid"><legend className="visually-hidden">Choose who should appear in the caricature</legend>{subjectChoices.map(item => <button type="button" aria-pressed={brief.subject === item.id} onClick={() => chooseSubject(item.id)} key={item.id}><b>{item.name}</b><span>{item.note}</span></button>)}</fieldset><fieldset className="number-fields" aria-describedby="subject-count-help"><legend>Confirm the number of subjects</legend><label><span>People</span><input type="number" inputMode="numeric" min="0" max="8" value={brief.people} onChange={event => patch('people', Number(event.target.value))}/></label><label><span>Pets</span><input type="number" inputMode="numeric" min="0" max="4" value={brief.pets} onChange={event => patch('pets', Number(event.target.value))}/></label><small id="subject-count-help">ArtzyAI preview supports up to 4 people and 2 pets.</small></fieldset><p className="subject-permission-note">Have permission to use every photograph. Separate photos can be combined after studio review.</p>{manual && <p className="manual-note" role="status"><strong>Studio-guided request</strong><span>This group is beyond the reliable preview range, so the studio will review it personally.</span></p>}</div>}
      {step === 2 && <div className="wizard-pane"><header><b>Step 3 · Type and style</b><h3>Choose the composition, then its visual mood</h3><p>Caricature type controls what is shown. Visual style controls how it feels.</p></header><h4>Caricature type</h4><div className="caricature-type-grid">{types.map(item => <button type="button" aria-pressed={caricatureType.id === item.id} onClick={() => setCaricatureType(item)} key={item.id}><b>{item.name}</b><span>{item.note}</span><small>Studio-priced for your brief</small></button>)}</div><h4>Visual style</h4><div className="caricature-style-grid">{visualStyles.map(option => { const item = CARICATURE_STYLES[option.id]; return <button type="button" aria-pressed={brief.styleId === option.id} className={brief.styleId === option.id ? 'is-selected' : ''} onClick={() => patch('styleId', option.id as CaricatureStyleId)} key={option.id}><img src={item.image} loading="lazy" alt={`${option.name} fictional style demonstration`}/><span><b>{option.name}</b><small>{item.summary}</small><em>Fictional AI sample</em></span></button>; })}</div></div>}
